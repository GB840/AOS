...
class TaskFingerprint(TimestampMixin, table=True):
    ...
    intent_summary: str | None = Field(default=None, sa_column=sa.Column(sa.Text))
...
class WorkflowDefinition(TimestampMixin, table=True):
    ...
    id: int | None = Field(default=None, primary_key=True)
    skill_id: int | None = Field(default=None, sa_column=sa.Column(sa.Integer, sa.ForeignKey("skills.id", ondelete="SET NULL")))
...
class EvolutionLog(TimestampMixin, table=True):
    ...
    from_version: str | None = Field(default=None, sa_column=sa.Column(sa.Text))
    to_version: str | None = Field(default=None, sa_column=sa.Column(sa.Text))
...
class LearningExperience(TimestampMixin, table=True):
    ...
    id: int | None = Field(default=None, primary_key=True)
    task_fingerprint_id: int | None = Field(default=None, sa_column=sa.Column(sa.Integer, sa.ForeignKey("task_fingerprints.id", ondelete="CASCADE"), index=True))
...
class ModelRoutingHistory(TimestampMixin, table=True):
    ...
    id: int | None = Field(default=None, primary_key=True)
    agent_id: str = Field(sa_column=sa.Column(sa.Text, sa.ForeignKey("agents.agent_id"), index=True))
    task_id: str | None = Field(default=None, sa_column=sa.Column(sa.Text, sa.ForeignKey("tasks.id")))
    requested_capability: str | None = Field(default=None, sa_column=sa.Column(sa.Text))
    selected_model: str | None = Field(default=None, sa_column=sa.Column(sa.Text, index=True))
    fallback_used: bool = Field(default=False, sa_column=sa.Column(sa.Boolean, server_default=sa.false()))
    latency_ms: int | None = Field(default=None, sa_column=sa.Column(sa.Integer))
...
class SelfModificationProposal(TimestampMixin, table=True):
    ...
    rationale: str | None = Field(default=None, sa_column=sa.Column(sa.Text))
