...
class TokenLedger(TimestampMixin, table=True):
    ...
    model: str | None = Field(default=None, sa_column=sa.Column(sa.Text, index=True))
...
class RewardEvent(TimestampMixin, table=True):
    ...
    reason: str | None = Field(default=None, sa_column=sa.Column(sa.Text))
...
class EconomicTransaction(TimestampMixin, table=True):
    ...
    id: int | None = Field(default=None, primary_key=True)
    agent_id: str = Field(sa_column=sa.Column(sa.Text, sa.ForeignKey("agents.agent_id"), index=True))
    budget_pool_id: int | None = Field(default=None, sa_column=sa.Column(sa.Integer, sa.ForeignKey("budget_pools.id", ondelete="SET NULL")))
    ...
    memo: str | None = Field(default=None, sa_column=sa.Column(sa.Text))
