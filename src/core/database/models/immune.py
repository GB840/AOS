...
class IdentityVault(TimestampMixin, table=True):
    ...
    aid: Optional[str] = Field(default=None, sa_column=sa.Column(sa.Text))
    public_key: Optional[str] = Field(default=None, sa_column=sa.Column(sa.Text))
...
class SandboxPolicy(TimestampMixin, table=True):
    ...
    id: Optional[int] = Field(default=None, primary_key=True)
    agent_id: Optional[str] = Field(default=None, sa_column=sa.Column(sa.Text, sa.ForeignKey("agents.agent_id"), index=True))
