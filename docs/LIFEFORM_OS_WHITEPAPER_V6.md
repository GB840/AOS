...
> **宪法原则的可验证承接（对齐 `AGENTS.md` §0.0.3「不收割四硬检验」）**：上述十条不是口号——凡新增功能 / 商业化动作，必须能过 **断网 / 出走 / 付费墙 / 抽成** 四条硬检验，任一条不过即判定「在收割」，须改。其可执行版本为 `tests/test_no_harvest_charter.py`（**16 项守门测试**，经反向验证真拦得住功能墙 / 锁定 / 抽成）。本白皮书所有能力声明与商业化边界，默认服从该守门器；代码层当前为 **零 AGPL 真接组件**（DBX 仅为纯参考、从未 import）。
