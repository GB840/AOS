...
| P1 | #95/#96 | 环境可复现 + 测试可收集 | ✅ `9258cf6` |
| P1 | #98 | 删除自研 OpenClaw 废弃代码 | ✅ `8826f8b` |
| P1 | #100/#101 | 路由收编 fabric + brain.py 拆分 | ✅ `c4a0ad8` (#100 已确认路由经 fabric 薄缝; #101 抽出 brain_registration.py) |
| P2 | #99/#102/#103 | 测试归位 / src/src 嵌套 / 运行时产物 | ✅ 验证: tests/ 已归位, 无 src/src 嵌套, 运行时产物(data/logs/__pycache__/*.db) 已 gitignore |
| P2 | #104 | 补提交建立版本保护 | ✅ 验证: .gitignore 已排除 .env*/cookies.txt/.secrets/, 无任何敏感文件入库 |
| P2 | #105/#106 | 校正 README / 统一表数 | ✅ README 38→42 (`c4a0ad8` 后续提交) |
| 待查 | #11 | 缺索引（表数实际 42，非 38） | ✅ 已核实：真实 42 表，断言已校正 (`9258cf6`) |
| 待查 | #13 | 依赖锁定（requirements.txt 有锁，pyproject 待确认） | ✅ 已核实：新增 requirements.lock 全量锁定 (`9258cf6`) |
