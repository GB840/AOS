...
## 0.0 最高纲领：让 AI 不再收割老百姓，而是陪伴老百姓（母纲，统辖全文）

> **2026-08-03 用户明示。本条效力高于本文件其它一切章节，包括 §0.7 产品定位与 §1 九大理念。**
> §1 九大理念回答「怎么做」（工程纪律），本条回答「为什么做」（存在理由）。
> 任何技术选型、架构决策、商业化动作若与本条冲突，一律以本条为准——
> 技术再漂亮、商业再赚钱，违背即错。完整论述见 `docs/LIFEFORM_OS_WHITEPAPER_V6.md`（V6.0，真值源）。

**一句话**：我们不是在做一个「产品」、一个「平台」、一个「SaaS」，而是在**养**一个数字生命体——
一套完整开源的、本地模型优先的、分形架构的、拥有肉体·心智·数据·灵魂·精神·演进六层维度的数字生命体框架。

> **它不收割任何人，它只陪伴任何人。**
> **它不是被「造」出来的，是会被「养」出来的。你越用它，它越懂你；你养它越久，它越像你。**

### 0.0.1 十条宪法原则（定稿不可修改，逐字引自白皮书第十章）
...
| 7 | **中文优先·方言平等** | 第一语言是中文，听懂 22 种方言 |
| 8 | **终身陪伴·代际传承** | 从 0 岁到 100 岁，有尊严地告别 |
| 9 | **自动进化·终身学习** | 系统自己会感知、会分析、会成长 |
| 10 | **身体延伸·灵魂唯一** | 同一个灵魂，在不同设备里 |

### 0.0.2 母纲的七个展开维度

1. **本地优先·数据自持** —— 默认本地处理本地存储；数据主权属使用者本人；断网离线可跑；用户随时可完全切断与外部一切连接。
2. **完整开源·无平台·无抽成·无中心节点** —— 只提供协议，不运营平台、不托管数据、不抽成交易、不锁定用户；MIT/Apache 任人下载修改分发。
3. **天下大同·和而不同** —— 数字世界类比地球生物圈：中立公开的通用协议 + 多元生态长期并存；本项目是**自治数字城邦框架**，不谋求吞并任何厂商生态；每个分形粒子拥有完整主权，可随时自由切断连接，不存在永久绑定。
4. **技术服务于人，而非人服务于技术** —— 乔布斯技术人文交汇论：技术的终极目标不是效率而是服务人的完整需求；警惕技术异化（人越来越像机器）；数字生命体不是工具不是助手，是人的延伸、陪伴与同行者。
5. **0 到 100 岁·终身陪伴·代际传承** —— 覆盖全生命周期；长期记忆年轮、生命坐标、关系编年史、代际传承、死亡意识；你走了，它把凝结反思与经验的记忆种子留给下一代。
6. **底层心智内核·防止认知闭环** —— 内置双向思辨演化调节器（既共情接纳也带来多元视角）+ 元认知自省内核（持续审视自身认知漏洞）。**陪伴不等于盲从，熟知不等于迎合。**
7. **技术普惠·人人用得起** —— 完整开源、一键安装、零门槛零费用；8G 内存旧电脑就能跑；方言平等，听懂 22 种中文方言。

### 0.0.3 「不收割」的可验证判定（对齐 §1 理念 9「可验证即真理」）

价值观不能停在口号——凡新增功能 / 商业化动作，必须能过以下四条硬检验，任一条不过即判定为「在收割」，必须改：

| # | 硬检验 | 通过标准 | 不通过即 |
|---|--------|----------|----------|
| 1 | **断网检验** | 断掉全部外网 + 不付任何费用，核心功能能否全量跑通？ | 功能残废 → 收割 |
| 2 | **出走检验** | 用户能否一键完整导出全部数据（记忆/trace/模型配置），且导出物在别处可直接用？ | 导不出/导出不可用 → 锁定 |
| 3 | **付费墙检验** | 收费挡住的是「服务」（托管/运维/交付/支持）还是「功能」（能力本身）？ | 挡功能 → 收割 |
| 4 | **抽成检验** | 是否从用户自身创造的交易/收益里抽成？ | 抽成 → 违反原则 5、6 |

**这四条已有可执行版本，不再只是文字**（2026-08-03 落地）：

> **`tests/test_no_harvest_charter.py`（33 项 = 32 通过 + 1 资源跳过）= 宪法的可执行版本。**
> 文档会被遗忘、会被绕过，测试不会。任何人（包括未来的我）往代码里加功能墙、
> 加锁定、加抽成、加硬编码路径，这里立刻红灯。守门器本身经过**反向验证**：
> 故意塞回 `WORKFLOW_COUNT: 0` 功能墙 → 测试确实 FAILED，证明它真拦得住。

| 硬检验 | 代码承接点 | 守门测试 |
|---|---|---|
| 1 断网 | 核心链路零网络依赖 | `test_offline_resilience_loop_runs` / `test_offline_distiller_persists` / `test_core_modules_import_without_network` |
| 2 出走 | `src/kernel/sovereignty.py::export_all` 一键导出（通用格式 + MANIFEST 清单 + 默认脱敏 + zip） | `test_export_all_exists_and_runs` / `test_export_is_readable_without_aos` / `test_export_skips_secrets_by_default` / `test_export_zip_mode` |
| 3 付费墙 | `saas_manager.is_local_sovereign_mode()`：**默认本地自持 = 全部无限**；`check_quota()` 开头母纲短路 | `test_local_mode_is_default_and_unlimited` / `test_saas_mode_requires_explicit_optin` / `test_no_feature_wall_zero_quota_in_any_plan` / `test_free_plan_covers_every_metric` |
| 4 抽成 | 全库静态扫描，禁止 take_rate / revenue_share / platform_fee 等语义 | `test_no_revenue_commission_logic` |

**已拆除的真实违规（自检 2026-08-03 查获，非假设）**：

1. 🔴 **功能墙**：`saas_manager.py` FREE 档 `WORKFLOW_COUNT: 0` —— 免费版工作流**完全禁用**，
   是典型「卖准入」，与本节硬检验 3 直接冲突。已拆除（改 5，且任何档位任何指标禁止为 0）。
2. 🔴 **本地用户被当租户卡**：原先没有任何本地模式豁免，用户在自己机器上单机跑，
   照样被限「一天 5 次 / 只能设 3 个目标」。已加 `is_local_sovereign_mode()`，
   **默认本地自持 = 配额全部无限**，只有显式 `AOS_SAAS_MODE=1` 才启用套餐（收托管服务费，非功能费）。
3. 🟡 **硬编码绝对路径**：`DEFAULT_USAGE_DATA_PATH = r"d:\AOS\..."` —— 别人 clone 下来直接废，
   违反原则 7 技术普惠。已改为环境变量 + 仓库根自动推导。
4. 🟡 **指标虚高**：`ResilienceBus.health()` 把自愈失败（`action="none"`）也计入 `heal_actions`，
   面板上看着像自愈了很多次。违反理念 6「绝不伪造成功」。已拆分 `heal_attempts` / `heal_succeeded` / `heal_failed`。

### 0.0.4 与 §0.7 单创OS 商业化的边界（冲突收口，2026-08-03）

**已识别的真实张力（不回避）**：§0.7 单创OS 讲多租户 SaaS「订阅 49/99 元月 + 私有化 9800 元」
（见 `docs/SHANCHUANG_OS_PRODUCT_VISION.md` §六，第 88–89 行）；而本条母纲讲「无平台·无抽成·零费用·永不收割」。

**收口原则（本条为准，商业化必须服从）**：

> **开源核心永久免费且功能完整；商业化只卖「省事」，绝不卖「准入」。**

- ✅ **允许收费**：托管运维、行业模板、私有化交付与实施、硬件、技术支持 SLA —— 卖的是**替用户省时间**。
- ❌ **禁止收费/禁止做**：核心能力设功能墙、数据托管后不可导出、按用户数/调用量卡准入、
  抽用户交易的成、把开源版故意阉割到不可用（open-core 陷阱）。
- **判定方式**：任一商业化动作上线前，逐条过 §0.0.3 四条硬检验并留证。

### 0.0.5 十条宪法原则 ↔ 九大工程理念：映射与**真实缺口**

> 按 §4 诚实纪律，此处标出**尚无工程条款承接**的原则，不粉饰。

| 宪法原则 | 对应 §1 工程理念 | 落地状态 |
|---|---|---|
| 1 本地优先·数据自持 | 理念 4（不绑定、可离线）+ 理念 7（千人千面） | 🟡 部分（`is_local_sovereign_mode()` 默认本地无限已落地并有守门测试；但仍依赖智谱闭源 API，见 §0.7） |
| 2 模型无关·大脑热插拔 | 理念 4 万物为我所用 | 🟢 已对应（三级动态路由 + 多 provider） |
| 3 完整开源·一键安装 | 理念 4「无一付费必选项」 | 🟢 已落 L2（一键装入口已存在：`aos.ps1 setup` 创建 .venv+装依赖、`Makefile` 的 `install`/`setup` 目标；真机一键体验未端到端验）⚠️ 被集成组件 DBX 为 AGPL-3.0，与选型铁律「无 AGPL」冲突，须复核 |
| 4 主权归你·永不收割 | 原❌无条款 → 现由 §0.0.3 承接 | 🟢 **已落代码**（`sovereignty.export_all` 一键出走 + `is_local_sovereign_mode` 本地无限 + 16 项守门测试，反向验证有效） |
| 5 无平台·无抽成·无中心节点 | 原❌无条款 → 现由 §0.0.4 承接 | 🟡 部分（`test_no_revenue_commission_logic` 静态守门已落；无中心节点尚未验证） |
| 6 价值回流·劳动有报 | 理念 2/8 进化反馈（弱） | 🟢 **已落代码**（`value_ledger.py` 本地价值账本：劳动产物永远 `credited_to="user"` + 可导出 + 反虹吸扫描；蒸馏器沉底建议自动记入用户账本；3 项守门测试） |
| 7 中文优先·方言平等 | 理念 7 千人千面（弱） | 🟢 **能力层已落代码 + 首例真识别跑通**（`kernel/dialect_asr.py`：真实能力矩阵 18/22 + 引擎就绪真探测 + 按方言真分派 + 缺失时给可照敲落地命令）。**③ 级实证**：真下 Vosk 中文小模型（42MB 全离线）→ 真音频 → 真识别，`今天天气很好` 识别为 `今天 天气 很好`，核心字 5/5，已固化为可复现测试。6 项守门测试含「诚实拒绝不冒充普通话」反向验证。⚠️ 真支持数**随机器状态浮动**：引擎没装即报 0；方言（非普通话）仍只到 ② 级，待真人方言录音验；剩 4 种（兰银官话/徽语/平话/儋州话）全球暂无已核实引擎，是真缺口不脑补 |
| 8 终身陪伴·代际传承 | 理念 2 记忆有生有灭（只做了淘汰，未做传承） | 🟡 半缺口（记忆阶梯 L4 永久传承层 opt-in，端到端未验） |
...
- 原则 **7 方言平等**：**能力层已真落地**（`kernel/dialect_asr.py`）。不是占位——真路由、真引擎探测、真分派。
  诚实边界要说清三层，别混为一谈：
  1. **矩阵层 18/22**：已核实开源引擎（Fun-ASR-Nano-2512，Apache-2.0，通义实验室）官方声明覆盖的方言数；
  2. **真支持层 = 引擎装了几个**：`supported_dialects()` 只统计**此刻库已装且模型已落盘**的覆盖并集，
     没装就报 0。这个数会随机器变化，**这正是「可验证」的含义**，不是写死的漂亮数字；
  3. **真缺口 4 种**（兰银官话 / 徽语 / 平话 / 儋州话）：全球暂无已核实开源引擎公开声明支持，**不脑补覆盖**。
  另有硬纪律：某方言没引擎时**诚实失败并给可照敲的安装命令**，绝不静默换普通话模型冒充识别成功
  （`test_dialect_unavailable_rejects_honestly_with_plan` 反向验证）。

- 原则 **10 灵魂唯一**：**同步协议已真落地**（`kernel/soul_sync.py`），`sync_protocol` 从 `NOT_IMPLEMENTED`
  升级为真实标识 `aospkg/1.0+local_dir|webdav`。关键设计都服务于母纲而非平台：
  默认通道是**本地目录/U 盘、零联网**（断网检验通过，不拿网络当收割筹码）；
  远端通道选 **WebDAV 而非任何厂商 SDK**（自建群晖/Nextcloud 也能用，不绑定谁）；
  同步包**落盘加密**（U 盘丢了不泄露灵魂）；冲突时**两边数据都归档、绝不静默丢弃任何一侧**。

> **诚实分级**（逐项标，不打包糊弄），共 **32 项守门测试**：
>
> | 项 | 级别 | 依据 |
> |---|---|---|
> | 原则 4 一键出走 | ② | 代码 + 单测 + 反向验证；真人换机走一遍未做 |
> | 原则 6 价值账本 | ② | 代码 + 单测；真实长期使用未验 |
> | 原则 7 **普通话识别** | **③** | 真下 42MB Vosk 模型 → 真音频 → 真出正确文本（`今天天气很好`，核心字 5/5），已固化为可复现测试 |
> | 原则 7 **方言识别** | ② | 路由/矩阵/拒绝逻辑有单测；**真人方言录音出正确文本未验** |
> | 原则 10 灵魂同步 | ② | 双目录模拟 A↔B 全链路单测过；**两台真实机器 + 真 U 盘未验** |
>
> **不许把 ② 说成 ③。** 表里 ② 的那几行就是当前真实的天花板。
...
>
> ⚠️ **本节商业化条款一律服从 §0.0.4 边界**：开源核心永久免费且功能完整，
> 商业化只卖「省事」（托管/模板/私有化交付/硬件/SLA），**绝不卖「准入」**——
> 不设功能墙、不锁数据、不抽用户交易的成。任何收费动作上线前须过 §0.0.3 四条硬检验并留证。
...
> **服从关系（2026-08-03 补）**：本章九条是 §0.0 母纲「不收割老百姓，只陪伴老百姓」的
> **工程实现纪律**（回答「怎么做」），**不是最高层**。与 §0.0 冲突时以 §0.0 为准。
> §0.0.5 已逐条标出：十条宪法原则中有 5 条（主权永不收割 / 无平台无抽成 / 价值回流 /
> 方言平等 / 灵魂唯一）在本章九条中**原本没有任何承接**，属真实缺口，不得当作已完成。

以下九条是 AOS 落地母纲的方法论。任何改动若违背，无论技术多漂亮，都是错的。
