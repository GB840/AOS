...
| 7 中文优先·方言平等 | 理念 7 千人千面（弱） | 🟢 **能力层已落代码**（`kernel/dialect_asr.py`：真实能力矩阵 18/22 + 引擎就绪真探测 + 按方言真分派 + 缺失时给可照敲落地命令；引擎为 Fun-ASR-Nano-2512 Apache-2.0 纯 CPU 离线 / Vosk 42MB 小模型。5 项守门测试含「诚实拒绝不冒充普通话」反向验证）。⚠️ 真支持数**随机器状态浮动**：引擎没装即报 0，剩 4 种（兰银官话/徽语/平话/儋州话）全球暂无已核实引擎，是真缺口不脑补 |
...
> **诚实分级**：原则 4 / 6 / 7 / 10 的落地均为 **② 级（代码 + 单测实证，含反向验证）**，共 30 项守门测试。
> ③ 级**未做**，具体指：真实用户在真机上完整走一遍（导出→换机导入→继续用；
> 真下方言模型→对真人方言录音出正确文本；两台真实机器经真 U 盘同步）。
> **不许把 ② 说成 ③。**
