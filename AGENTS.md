...
| 7 中文优先·方言平等 | 理念 7 千人千面（弱） | 🟡 部分（**诚实缺口追踪器** `constitution_gaps.dialect_summary` 如实报「支持 0 / 目标 22」，绝不说谎；反虚假宣称闸门 `test_no_fake_dialect_claim` 拦住任何假支持。真方言 ASR 模型仍未集成） |
| 8 终身陪伴·代际传承 | 理念 2 记忆有生有灭（只做了淘汰，未做传承） | 🟡 半缺口（记忆阶梯 L4 永久传承层 opt-in，端到端未验） |
| 9 自动进化·终身学习 | 理念 2 + 2.5 + 8 | 🟡 代码在，③ 级端到端未验 |
| 10 身体延伸·灵魂唯一 | 理念 3 芯粒隔离（讲的是进程隔离，非跨设备同一灵魂） | 🟡 部分（**灵魂 ID 原语** `constitution_gaps.get_or_create_soul_id`：本地稳定、可随导出带走、可移植；反虚假宣称闸门 `test_soul_sync_protocol_not_faked` 诚实标注同步协议 `NOT_IMPLEMENTED`。真跨设备同步协议未做） |
...
- 原则 **7 方言平等 / 10 灵魂唯一**：**真能力（方言 ASR / 跨设备同步）仍未做**，但已搭建「诚实缺口追踪 + 反虚假宣称闸门」——
  如实报 0/22、诚实标 `NOT_IMPLEMENTED`，任何假支持都会立刻被守门测试拦红。**绝不粉饰为已完成**。

> **诚实分级**：本节所述原则 4 的落地为 **② 级（代码 + 单测实证，含反向验证）**。
> ③ 级（真实用户在真机上完整走一遍导出→换机导入→继续用）**未做**。
