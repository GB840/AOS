...
这五条是 AOS 存在的理由。任何改动若违背，无论技术多漂亮，都是错的：
...
5. **极简优先（Ponytail 阶梯）** — 任何改动/方案，能简单、高效、高质量、安全搞定就别搞复杂。
   写代码、定架构、写文档、定流程、选依赖前，先跑 §10 的七级决策阶梯；停在第一个成立的台阶。
   > 溯源：已按用户铁律全网搜证核实的开源项目 Ponytail（github.com/DietrichGebert/ponytail，MIT）。
   > 懒，但不渎职（Lazy, not negligent）：信任边界 / 数据丢失 / 安全 / 可访问性 永不砍。
